...
def predict_drawing(image):
	...
	"""kernel = np.ones((5,5), np.uint8) #This is only used for handwritten dataset when the paintbrush stroke was 3px
	thickened = cv2.dilate(inverted, kernel, iterations=1)

	resized = cv2.resize(thickened, (28,28), interpolation=cv2.INTER_AREA)"""
